...
NS_LOG_COMPONENT_DEFINE("s1g-wifi-network-tim-raw-cap");
...
float getDataRate(string dataMode) {
	if (dataMode == "MCS1_0")
		return 0.3;
	else if (dataMode == "MCS1_1")
		return 0.6;
...
int main(int argc, char *argv[]) {
	...
	config.datarate = getDataRate(config.DataMode);
...
*/
	...
	if (config.eh == -1) { // sunny 
		enableVariableHarvester = true;
		filenameHarvester = config.pathToInputFile + config.filenameHarvesterSun;
	} else if (config.eh == -2) { // cloudy
		enableVariableHarvester = true;
		filenameHarvester = config.pathToInputFile + config.filenameHarvesterCloudy;
	...
	double minPowerDensity = config.eh; // 30e-3;
	double maxPowerDensity = config.eh; // 0.30e-3;
	...
	EnergySourceContainer sources = capacitorHelper.Install (wifiStaNode);
	// install device model
	DeviceEnergyModelContainer deviceModels = radioEnergy.Install (staDevice, sources);
